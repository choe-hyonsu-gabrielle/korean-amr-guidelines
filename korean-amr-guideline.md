...
이 AMR의 의미는 대략 다음과 같습니다: 사건 `생각-01` 주체(`:ARG0`)는 `소년`이 되고 대상(`:ARG1`)은 `좋-01` 사건이 됩니다. (*소년이 '~ 좋아한다'고 생각한다.*) `좋-01` 사건의 주체(`:ARG0`)는 `소녀`가 되고 대상(`:ARG1`)은 앞서 상위에서 언급되었던 `소년`이 됩니다. (*소년이 '소녀가 소년을 좋아한다'고 생각한다.*) 따라서 `소년`의 역할은 (1) `생각-01` 사건의 주체(`:ARG0`)이자 (2) `좋-01` 사건의 대상(`:ARG1`)으로 모두 두 가지입니다. 이런 경우 AMR에서는 같은 개념 단말을 가리키는 두 개의 관계 화살표를 통해 표현할 수 있습니다.
...
instance(좋, 좋-01) ^         /* '좋'은 '좋-01'의 인스턴스 */
...
`좋-01`은 *"좋아한다"*, *"좋아하다"*, *"좋아할 것이다"* 등으로 실현될 수 있을 것입니다. AMR에서 단말은 실제로 쓰인 '단어(Word)'가 아니라 '개념(Concept)'입니다. AMR의 요소(element)를 가리키면서 *"이것은 동사다"*, *"이것은 명사다"*와 같이 말하지 않습니다. 오히려 *"이것은 객체(object)다."* 또는 *"이것은 사건(event)이다"*와 같이 말할 수 있습니다.

단일 개체(entity)인 `소년`은 `생각-01`의 `:ARG0`나 `좋-01`의 `:ARG1`과 같이 동시에 여러 역할을 수행할 수 있습니다. AMR은 대명사나 영 대명사 (zero-pronouns)에 대해서는 언급하지 않지만, 한국어에서 이러한 방식의 대용은 한 문장에서 어떤 개체가 여러 역할을 수행하는 것을 표현하는 자연스러운 방식입니다.
