...
보조사 '-은/는'과 주격 조사 '-이/가'는 분포가 상당히 비슷합니다. 그렇다면 "와인이 향이 좋다"는 어떻게 표상해야 할까요?

우선 "와인"이 특정한 대상을 가리키는 경우라면 (예: 발화자의 눈 앞에 있는 와인 한 잔) 아래와 같이 표상하는 것이 바람직합니다. 바꿔 말하면 "와인"이 클래스(class)인 경우와 인스턴스(instance)인 경우는 구분되어야 합니다.
...
그러나 "와인은 (일반적으로) 향이 좋다."의 의미라면 '와인이 어떠하다' + '향이 좋다'로 봅니다.
...
간혹 보조 용언의 여부가 본 용언의 의미를 크게 바꾸지 않는 경우도 있습니다. 예를 들어 KPB에는 `차-01`(fill)과 `채워넣-01`(fill)이 모두 있는데, 이 경우에는 보조 용언이 없는 `차-01`을 사용하는 것을 원칙으로 합니다.
